...
// GET /api/dashboard-data/all - Get all dashboard data in one request
router.get('-data/all', async (req, res) => {
    try {
        // Fetch log level stats (24h)
        const logLevelStats = await req.dal.all(`
            SELECT level, COUNT(*) as count 
            FROM logs 
            WHERE timestamp >= NOW() - INTERVAL '24 hours'
        ...
        // Fetch hourly stats (24h)
        const hourlyStatsRaw = await req.dal.all(`
            SELECT 
        ...
        // Format hourly stats for ECharts
        ...
        // Get integration stats (if available)
        let integrationStats = [];
        try {
            const integrations = await req.dal.all(`SELECT * FROM integrations WHERE enabled = true LIMIT 10`) || [];
            integrationStats = integrations.map(int => ({
                name: int.name,
                type: int.type,
        ...
        console.error('[Dashboard] /all endpoint error:', error);
        req.app.locals?.loggers?.api?.error('API dashboard-data/all error:', error);
