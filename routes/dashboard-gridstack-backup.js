...
router.get('/', async (req, res) => {
    try {
        ...
        try {
            ...
            req.app.locals?.loggers?.system?.error('Error getting system stats:', error);
        }
        
        try {
            recentLogs = await req.dal.getRecentLogs(20) || [];
        } catch (error) {
            req.app.locals?.loggers?.system?.error('Error getting recent logs:', error);
        }
        
        try {
            systemHealth = await req.dal.getSystemHealth() || systemHealth;
        } catch (error) {
            req.app.locals?.loggers?.system?.error('Error getting system health:', error);
        }
        
        try {
            logSources = await req.dal.getLogSources() || [];
        } catch (error) {
            req.app.locals?.loggers?.system?.error('Error getting log sources:', error);
        }
        ...
        try {
            ...
            req.app.locals?.loggers?.system?.error('Error getting user dashboards:', error);
            userDashboards = { success: false, data: [] };
        }
    ...
    const contentBody = `
        <!-- Include Muuri and Apache ECharts -->
        ...
        .widget-item {
            ...
            z-index: 1;
            cursor: move;
            transition: none; /* Muuri handles animations */
        }
        ...
        .widget-small { width: 300px; height: 250px; }
        .widget-medium { width: 400px; height: 350px; }
        .widget-large { width: 600px; height: 400px; }
        .widget-wide { width: 800px; height: 300px; }
        .widget-full { width: calc(100% - 10px); height: 400px; }
        </style>
        ...
        <div class="dashboard-grid" id="dashboardGrid">
            ...
            <!-- System Stats Widget -->
            <div class="widget-item widget-full" data-id="system-stats">
                <div class="grid-stack-item-content">
                    <div class="widget-card">
                        ...
                        <div class="widget-content" id="systemStatsContent">
            ...
            <div class="widget-item widget-medium" data-id="log-levels-chart">
                <div class="widget-item-content">
                    <div class="widget-card">
                        ...
                        <div class="widget-content">
                            <div id="logLevelsChartContainer">
    ...
    const additionalJS = `
    // Dashboard Configuration (real-time handled by global Realtime registry)
    let autoRefreshEnabled = true; // UI state only; actual interval now via Realtime task
    let refreshInterval; // legacy variable retained for backward compatibility (no longer used)
        let grid;
        let charts = {};
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            req.app.locals?.loggers?.system?.info('🎯 Dashboard initialized with real-time widgets');
        });
        ...
        function initializeCharts() {
            // EMPTY STATE HANDLING: Only initialize charts/gauges if data exists
            const pieData = ${JSON.stringify(logLevelStats)};
            if (!pieData || pieData.length === 0) {
                showEmptyState('logLevelsChartContainer', 'No log events in last 24h', 'chart-pie');
            } else {
                initializeLogLevelsChart();
            }

            const health = ${JSON.stringify(systemHealth)};
            const hasHealthData = health && ((health.cpu || 0) + (health.memory || 0) + (health.disk || 0)) > 0;
            if (!hasHealthData) {
                showEmptyState('metricsGrid', 'No system metrics available', 'tachometer-alt');
            } else {
            ...
            if (!hourlyData || hourlyData.length === 0) {
                showEmptyState('timelineChartContainer', 'No logs recorded in last 24 hours', 'chart-line');
            } else {
                initializeTimelineChart();
            }

            if (${recentLogs.length} === 0) {
                showEmptyState('recentLogsContainer', 'No recent activity', 'list');
            }

            if (${stats.totalLogs || 0} === 0 && ${logSources.length} === 0) {
                showEmptyState('systemStatsContent', 'No system statistics available', 'chart-bar');
            }
        }
        ...
        function startAutoRefresh() {
            // Delegate to Realtime registry; ensure task registered if not present
            if (autoRefreshEnabled && window.Realtime) {
                if (!window.Realtime.isEnabled()) window.Realtime.enable();
                // Task is auto-registered from base template; just update UI
        ...
        function toggleAutoRefresh() {
            ...
            if (autoRefreshEnabled) {
                ...
                // Disable registry task globally only if no other realtime desired
                if (window.Realtime) window.Realtime.disable();
                btn.textContent = 'Auto Refresh: OFF';
                btn.insertAdjacentHTML('afterbegin', '<i class="fas fa-sync"></i> ');
                btn.classList.remove('auto-refresh-active');
            } else {
                autoRefreshEnabled = true;
                if (window.Realtime) window.Realtime.enable();
        ...
        async function refreshAllWidgets() {
            try {
                ...
                const data = await apiFetch('/api/dashboard/refresh');
                if (data) {
                    updateDashboardData(data);
                    document.getElementById('lastUpdate').textContent = new Date().toLocaleTimeString();
                }
            } catch (error) {
                req.app.locals?.loggers?.system?.error('Auto-refresh failed:', error);
            }
        }
        ...
        function updateDashboardData(data) {
            // Update stats
            if (data.stats) {
                document.getElementById('totalLogs').textContent = (data.stats.totalLogs || 0).toLocaleString();
                document.getElementById('logsToday').textContent = (data.stats.logsToday || 0).toLocaleString();
            }
            ...
            if (data.logSources) {
                ...
                if (sourcesEl) {
                    sourcesEl.textContent = data.logSources.length || 0;
                }
            }

            // Update system health numbers and gauges (keep UI and visualizations in sync)
            if (data.systemHealth) {
                const cpu = Math.max(0, Math.min(100, Math.round(data.systemHealth.cpu || 0)));
                const mem = Math.max(0, Math.min(100, Math.round(data.systemHealth.memory || 0)));
                const disk = Math.max(0, Math.min(100, Math.round(data.systemHealth.disk || 0)));
                const status = data.systemHealth.status || 'unknown';
                ...
                const healthBadges = document.querySelectorAll('.stat-item .status-badge');
                healthBadges.forEach(badge => {
                    badge.className = 'status-badge ' + status;
                    badge.textContent = status.toUpperCase();
                });

                // Update gauges if initialized
                updateGauge('cpuGauge', cpu);
                updateGauge('memoryGauge', mem);
        ...
        function updateGauge(canvasId, value) {
            ...
            chart.data.datasets[0].backgroundColor[0] = v > 80 ? '#ef4444' : v > 60 ? '#f59e0b' : '#10b981';
        ...
        window.addEventListener('beforeunload', function() {
            // Legacy cleanup no longer needed; registry handles shutdown
            if (refreshInterval) { clearInterval(refreshInterval); }
        });
        `;

        // escapeHtml() and formatDate() imported from utils/html-helpers
...
router.get('/api/refresh', async (req, res) => {
    try {
        ...
        req.app.locals?.loggers?.system?.error('Dashboard refresh API error:', error);
        res.status(500).json({ 
            success: false, 
...
// Load Generator for Testing - Creates activity to see gauges update
router.post('/api/test/generate-load', async (req, res) => {
    try {
        const { logs = 10, cpuBurn = 1000 } = req.body;
        
        // Generate test log entries
        const levels = ['info', 'warning', 'error', 'debug'];
        const sources = ['test-generator', 'load-test', 'stress-test'];
        ...
        for (let i = 0; i < logs; i++) {
            ...
            await req.dal.run(
                ...
                [new Date().toISOString(), level, `Test log #${i} generated for load testing`, source]
            );
        }
        
        // Generate CPU activity (burn cycles for a short time)
        const startTime = Date.now();
        let iterations = 0;
        while (Date.now() - startTime < cpuBurn) {
        ...
        res.json({
            success: true,
            logsGenerated: logs,
            cpuIterations: iterations,
            durationMs: Date.now() - startTime
        });
        
    } catch (error) {
        req.app.locals?.loggers?.system?.error('Load generation error:', error);
        res.status(500).json({ 
            success: false, 
            error: error.message 
        });
