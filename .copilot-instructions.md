...
- **Web UI Port**: `http://192.168.222.3:10180` (host port 10180 → container port 10180)
- **WebSocket Port**: `8081`
- **Stream Port**: `8082`
...
- Port mappings: `-p HOST_PORT:CONTAINER_PORT`
...
- 📖 **Quick Start Guide**: See `UNRAID_QUICKSTART.md` for 5-minute setup
- 📖 **Full Documentation**: See `UNRAID_SETUP.md` for comprehensive guide
- 🚀 **Auto-Deploy Script**: Use `unraid-deploy.sh` for automated installation
- 📦 **Template**: Use `unraid-template.xml` for Community Applications

### Development Mode - Bind Mounts
The Docker container has **bind mounts** to the local workspace, meaning:
- ✅ Code changes on your local machine are **immediately reflected** in the container
- ✅ No rebuild required for code changes
- ⚠️ After editing files, you must reload the app with `pm2 reload all` inside the container
- 📁 Mounted directories: `routes/`, `public/`, `views/`, `utils/`, `configs/`, and `data/`
...
1. **"Can't connect"** → Check Docker is running: `docker ps`
2. **"Port conflict"** → Container uses host port 10180, not 3000
3. **"Changes not reflected"** → Reload: `docker exec rejavarti-logging-server pm2 reload all`
4. **"Container won't start"** → Check logs: `docker logs rejavarti-logging-server`
5. **"Database errors"** → Verify bind mounts: `docker exec rejavarti-logging-server ls -la /app/data`

## What NOT to Do

❌ Never run `npm start` on the host  
❌ Never create scripts that run Node directly outside Docker  
❌ Never assume port 3000 is accessible on localhost (use 10180)  
❌ Never modify code without reloading the container  
❌ Never suggest recreating features that were explicitly removed (Dashboard Builder)  
❌ Never use CDN links (all assets are vendored locally)  
❌ Never use macvlan networking (bridge only)
❌ Never create placeholder/stub implementations - all features must be fully functional
❌ Never add buttons without working backend endpoints
...
### New Core Tables
- `transaction_log`: Forensic record of all multi-step operations (status: pending → committed / rolled_back / failed).
...
### Retry Backoff Logic
Retry intervals use exponential backoff: `2^attempt * 60s` until `max_retries` reached (then status → `abandoned`).
